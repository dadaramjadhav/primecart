...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(
        ...
        log.warn("Payment database constraint violation: method={}, path={}", request.getMethod(), request.getRequestURI());

        return buildResponse(HttpStatus.CONFLICT, "PAYMENT_DATA_CONFLICT", "The payment request conflicts with existing data", List.of(),
                             request);
    }
    ...
    public ResponseEntity<ErrorResponse> handlePaymentNotFound(PaymentNotFoundException exception, HttpServletRequest request) {
        return buildResponse(HttpStatus.NOT_FOUND, "PAYMENT_NOT_FOUND", exception.getMessage(), List.of(), request);
    }

    @ExceptionHandler(InvalidPaymentStateException.class)
    public ResponseEntity<ErrorResponse> handleInvalidPaymentState(InvalidPaymentStateException exception, HttpServletRequest request) {
        List<String> details = List.of("paymentId: " + exception.getPaymentId(), "currentStatus: " + exception.getCurrentStatus(),
                                       "operation: " + exception.getOperation());

        return buildResponse(HttpStatus.CONFLICT, "INVALID_PAYMENT_STATE", exception.getMessage(), details, request);
    }

    @ExceptionHandler(DownstreamAuthenticationException.class)
    public ResponseEntity<ErrorResponse> handleDownstreamAuthentication(
            DownstreamAuthenticationException exception,
            HttpServletRequest request
    ) {
        log.warn("Service-to-service authentication failed: method={}, path={}", request.getMethod(), request.getRequestURI());

        return buildResponse(HttpStatus.SERVICE_UNAVAILABLE, "DOWNSTREAM_AUTHENTICATION_FAILED",
                             "Unable to authenticate with a required downstream service", List.of(), request);
    }

    @ExceptionHandler(FeignException.NotFound.class)
    public ResponseEntity<ErrorResponse> handleOrderNotFound(FeignException.NotFound exception, HttpServletRequest request) {
        return buildResponse(HttpStatus.NOT_FOUND, "ORDER_NOT_FOUND", "The associated order was not found", List.of(), request);
    }

    @ExceptionHandler(FeignException.Conflict.class)
    public ResponseEntity<ErrorResponse> handleOrderStateConflict(FeignException.Conflict exception, HttpServletRequest request) {
        return buildResponse(HttpStatus.CONFLICT, "ORDER_STATE_CONFLICT",
                             "The order cannot accept this payment operation in its current state", List.of(), request);
    }

    @ExceptionHandler(RetryableException.class)
    public ResponseEntity<ErrorResponse> handleOrderServiceUnavailable(RetryableException exception, HttpServletRequest request) {
        log.warn("Order Service unavailable: method={}, path={}, exceptionType={}", request.getMethod(), request.getRequestURI(), exception
                .getClass()
                .getSimpleName());

        return buildResponse(HttpStatus.SERVICE_UNAVAILABLE, "ORDER_SERVICE_UNAVAILABLE", "Order Service is temporarily unavailable",
                             List.of(), request);
    }

    @ExceptionHandler(FeignException.class)
    public ResponseEntity<ErrorResponse> handleOrderServiceFailure(FeignException exception, HttpServletRequest request) {
        log.warn("Order Service request failed: method={}, path={}, downstreamStatus={}", request.getMethod(), request.getRequestURI(),
                 exception.status());

        return buildResponse(HttpStatus.BAD_GATEWAY, "ORDER_SERVICE_FAILURE", "Unable to synchronize the payment with Order Service",
                             List.of(), request);
    }
