...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(
        ...
        log.warn("Inventory database constraint violation: method={}, path={}", request.getMethod(), request.getRequestURI());

        return buildResponse(HttpStatus.CONFLICT, "INVENTORY_DATA_CONFLICT", "The inventory request conflicts with existing data",
                             List.of(), request);
    }
    ...
    public ResponseEntity<ErrorResponse> handleInventoryNotFound(InventoryNotFoundException exception, HttpServletRequest request) {
        return buildResponse(HttpStatus.NOT_FOUND, "INVENTORY_NOT_FOUND", exception.getMessage(), List.of(), request);
    }
    ...
    public ResponseEntity<ErrorResponse> handleInsufficientStock(InsufficientStockException exception, HttpServletRequest request) {
        List<String> details = List.of("productId: " + exception.getProductId(), "availableQuantity: " + exception.getAvailableQuantity(),
                                       "requestedQuantity: " + exception.getRequestedQuantity());

        return buildResponse(HttpStatus.CONFLICT, "INSUFFICIENT_STOCK", exception.getMessage(), details, request);
    }

    @ExceptionHandler(InsufficientReservedStockException.class)
    public ResponseEntity<ErrorResponse> handleInsufficientReservedStock(
            InsufficientReservedStockException exception,
            HttpServletRequest request
    ) {
        List<String> details = List.of("productId: " + exception.getProductId(), "reservedQuantity: " + exception.getReservedQuantity(),
                                       "requestedQuantity: " + exception.getRequestedQuantity());

        return buildResponse(HttpStatus.CONFLICT, "INSUFFICIENT_RESERVED_STOCK", exception.getMessage(), details, request);
    }
